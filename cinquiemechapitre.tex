...
Les données fournies pour un modèle IRT peuvent être déclarées comme suit pour tenir compte du fait que tous les étudiants ne sont pas tenus de répondre à toutes les questions.
\begin{lstlisting}[language=Stan,basicstyle=\scriptsize\color{white},backgroundcolor = \color{blueforest}]
data {
// numbers of things
...
\end{lstlisting}
Dans cette déclaration il y’a le nombre R un entier positif qui est le nombre d’observation, où pour toute valeus n de 1 à R grade[n] est la reponse obtenu par l’etudiant subject[n] à la question item[n].
\subsubsection{Les paramètres du model de Rasch (1PL)}
Le modèle IRT 1PL, également connu sous le nom de modèle Rasch, a un paramètre (1P) pour les questions (items).
Les paramètres du modèle sont déclarés comme suit.
...
\end{lstlisting}
Le paramètre ability[J] est le coefficient de capacité de l'élève Si et difficulty[I] le coefficient de difficulté de la question Ii. La paramétrisation non standard utilisée ici comprend également un terme d'interception delta, qui représente la réponse moyenne de l'étudiant à la question moyenne. [10] 
